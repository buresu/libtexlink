...
int main(int argc, char **argv) {
  if (argc < 2) {
    ...
    int n = texlink_list_sessions(names, 16);
    if (n == 0) {
  ...
  texlink_session_t *s = texlink_connect_by_name(name);
  if (!s) {
  ...
  texlink_meta_t meta = texlink_session_meta(s);
  printf("Buffer: %u bytes  format=0x%x\n", meta.size, meta.format);

  while (1) {
    int idx = texlink_consumer_acquire(s);
    if (idx < 0) {
      fprintf(stderr, "Acquire failed (producer disconnected?)\n");
      break;
    }

    texlink_buf_t *buf = texlink_session_buf(s, idx);
    texlink_meta_t cur_meta = texlink_session_meta(s);
    uint64_t *data = texlink_map_cpu(buf);
    if (data) {
      texlink_cpu_begin(buf, 0);
      uint64_t val = data[0];
      texlink_cpu_end(buf, 0);
      printf("frame=%" PRIu64 "  buf=%d  val=%" PRIu64 "\n", cur_meta.frame_id,
             idx, val);
    }

    texlink_consumer_release(s, idx);
  }

  texlink_session_close(s);
  return 0;
}
